...
internal sealed class CounterOutboxStore(CounterDbContext dbContext)
    : ICounterOutboxStore
{
        ...
        CancellationToken cancellationToken)
    {
        ...
        const string sql =
            ...
            UPDATE counter.outbox_messages AS message
            SET "LeaseId" = @leaseId,
        ...
        CancellationToken cancellationToken)
    {
        if (!string.Equals(safeErrorCode, PublishFailed, StringComparison.Ordinal))
        {
            throw new ArgumentException(
                "The outbox error code is not on the safe allow-list.",
