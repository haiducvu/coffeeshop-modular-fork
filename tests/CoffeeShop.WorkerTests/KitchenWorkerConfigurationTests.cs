...
public sealed class KitchenWorkerConfigurationTests
{
    [Fact]
    public void Missing_kitchen_connection_string_is_rejected()
    {
        ...
        var exception = Assert.Throws<OptionsValidationException>(() =>
            new ServiceCollection().AddKitchenWorker(configuration));

        var failure = Assert.Single(exception.Failures);
        Assert.Contains(
            "ConnectionStrings:Kitchen",
            failure,
    ...
    public void Missing_kafka_bootstrap_servers_is_rejected_before_migration()
    {
        ...
        services.AddLogging();
        services.AddKitchenWorker(configuration);
        using var provider = services.BuildServiceProvider();

        var exception = Assert.Throws<OptionsValidationException>(() =>
            provider.ValidateKitchenWorkerOptions());
    ...
    {
        var services = new ServiceCollection();
        services.AddLogging(logging => logging.AddKitchenWorkerLogging());
        using var provider = services.BuildServiceProvider();
    ...
    [Fact]
    public void Worker_registers_exactly_one_logical_kitchen_consumer_role()
    {
        var services = new ServiceCollection();
        services.AddKitchenWorker(ValidConfiguration(
        ...
        IReadOnlyDictionary<string, string?> overrides)
    {
        var settings = new Dictionary<string, string?>
        {
            ...
            ["Messaging:Kafka:TopicPrefix"] = "lesson32",
            ["Messaging:Kafka:ConsumerGroupPrefix"] = "lesson32"
        };
